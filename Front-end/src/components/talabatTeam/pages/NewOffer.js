...
class NewOffer extends React.Component {
  ...
  saveToLocalStorage = () => {
    localStorage["selected-items-offer"] = JSON.stringify(this.state.selectedItems); //to JSON
  };

  componentDidMount() {
    if (localStorage["selected-items-offer"]) {
      let selectedItems = JSON.parse(localStorage["selected-items-offer"]); // from JSON to string
  ...
  removeSelected = (item) => {
    ...
    if (localStorage["selected-items-offer"]) {
      let selected = JSON.parse(localStorage["selected-items-offer"]);
      if (item.quantity > 1) {
  ...
  //momkn adefhom lel localstorage law 3yza azhrhom fl copouns b7a2 w 72e2i 
    this.state.submittedOffers.push({restaurant_id: this.props.match.params.id,
      offerName:this.state.offerName,
      Price:this.state.offerPrice
    });
    this.state.submittedOffers.push(this.state.selectedItems);
   localStorage["submittedOffers"] = JSON.stringify(this.state.submittedOffers);
//   for(let i = 0; i < localStorage["selected-items-offer"]; i++) {
//   localStorage["selected-items-offer"].pop();
// }
  ...
  render() {
    return (
      <div className="container ">
        ...
        >
          Add New offer to [restaurant id: {this.props.match.params.id}]
        </h1>
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <a  href="/offers" class="btn" >Close</a>
