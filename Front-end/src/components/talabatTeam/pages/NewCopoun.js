...
class  NewCopoun extends React.Component {
      ...
      for (var i = 0; i < this.state.selectedItems.length; i++) { 
        if (this.state.selectedItems[i].id === item.id) {
          ...
          this.state.selectedItems[i].quantity++;
              console.log(
                "added to quantity of " + this.state.selectedItems[i].name);
        }
  ...
  saveToLocalStorage = () => {
    localStorage["selected-items-cop"] = JSON.stringify(this.state.selectedItems); //to JSON
  };

  componentDidMount() {
    if (localStorage["selected-items-cop"]) {
      let selectedItems = JSON.parse(localStorage["selected-items-cop"]); // from JSON to string
  ...
  removeSelected = (item) => {
    ...
    if (localStorage["selected-items-cop"]) {
      let selected = JSON.parse(localStorage["selected-items-cop"]);
      if (item.quantity > 1) {
  ...
  //momkn adefhom lel localstorage law 3yza azhrhom fl copouns b7a2 w 72e2i 
    this.state.submittedCopouns.push({restaurant_id: this.props.match.params.id,
      copounCode:this.state.copounCode,
      priceOff:this.state.priceOff
    });
    this.state.submittedCopouns.push(this.state.selectedItems);
   localStorage["submittedCopouns"] = JSON.stringify(this.state.submittedCopouns);
//   for(let i = 0; i < localStorage["selected-items-cop"]; i++) {
//   localStorage["selected-items-cop"].pop();
// }
   localStorage.removeItem('selected-items-cop');
  ...
  render() {
    return (
      <div className="container ">
        ...
        >
          Add New Copoun to [restaurant id: {this.props.match.params.id}]
        </h1>
          ...
          <form>
            ...
            <div style={{display:"flex"}}>
              <input type="text" class="form-control"   style={{width:"60px"}}
               ...
               onChange={(e)=>this.setState({priceOff:e.target.value})}/>
              <div> L.E Off Selected Items</div> </div>
            </div>
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <a  href="/copouns" class="btn" >Close</a>
